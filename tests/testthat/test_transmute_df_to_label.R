...
test_that(
  ...
  , {
    ...
    fused_2 <- transmute_df_to_label(t_out2)
    expect_identical(
    ...
    fused_out <- transmute_df_to_label(between_anova_out)
    expect_identical(
    ...
    expect_error(
      transmute_df_to_label(between_anova_out)
      , fixed = TRUE
    ...
    expect_error(
      transmute_df_to_label(multiple_df)
      , regexp = "Degrees of freedom (in column 'df') vary across table rows."
      , fixed = TRUE
    )
    ...
    expect_warning(
      transmute_df_to_label(noninteger_df)
    ...
    junk <- transmute_df_to_label(apa_no_df, check_df = FALSE)
