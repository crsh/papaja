...
apa_beeplot(
  ...
  , args_points = list(
    pch = rep(21, 3)
    , bg = c("red", "green", "orange")
  )
  , args_error_bars = list(
    col = "red"
    , lwd = 1:3
  )
)
...
```{r eval = FALSE}
papaja:::apa_plot.formula(formula = first_dv ~ A * C + (1 | sid), data = example_data) |> 
  violins(bw = .3)  |>
  error_bars() |>
  lines() |>
  points() |>
...
apa_beeplot(formula = first_dv ~ (A * B | sid), data = example_data)|>
  ...
  # annotation(xlab = "blabla") |>
  # annotation(ylab = c("Test", "blub")) |>
