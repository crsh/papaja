...
test_that(
  ...
  , {
    ...
    # expect_equal(tw_rm_lsm_output$stat$Task, "$F(1, 4) = 7.35$, $\\mathrm{MSE} = 4.08$, $p = .054$")
    # expect_is(tw_rm_lsm_output$stat$Valence, "character")
    # expect_equal(tw_rm_lsm_output$stat$Valence, "$F(2, 8) = 1.46$, $\\mathrm{MSE} = 3.36$, $p = .288$")
    ...
    # expect_equal(tw_rm_lsm_output$est$Task, "$\\eta^2_G = .068$")
    # expect_is(tw_rm_lsm_output$est$Valence, "character")
    # expect_equal(tw_rm_lsm_output$est$Valence, "$\\eta^2_G = .023$")
    ...
    # expect_equal(tw_rm_lsm_output$full$Task, "$F(1, 4) = 7.35$, $\\mathrm{MSE} = 4.08$, $p = .054$, $\\eta^2_G = .068$")
    # expect_is(tw_rm_lsm_output$full$Valence, "character")
    # expect_equal(tw_rm_lsm_output$full$Valence, "$F(2, 8) = 1.46$, $\\mathrm{MSE} = 3.36$, $p = .288$, $\\eta^2_G = .023$")
    # expect_is(tw_rm_lsm_output$full$Task_Valence, "character")
    # expect_equal(tw_rm_lsm_output$full$Task_Valence, "$F(2, 8) = 0.29$, $\\mathrm{MSE} = 2.41$, $p = .755$, $\\eta^2_G = .003$")
