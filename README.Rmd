...
dependencies <- packageDescription("papaja", fields = "Imports")
dependencies <- unlist(strsplit(dependencies, split = "\n"))[-1]
dependencies <- gsub("\\n| \\(.+\\),|,", "", dependencies)
dependency_graph <- miniCRAN::makeDepGraph(dependencies, suggests = FALSE, enhances = FALSE)
class(dependency_graph) <- "igraph"
dependency_graph <- dependency_graph + vertices("papaja") + edges(as.vector(rbind(dependencies, "papaja")))
dependency_graph <- simplify(dependency_graph)

edge_list <- get.edgelist(dependency_graph)
dependency_graph <- graph(rbind(edge_list[, 2], edge_list[, 1]))
...
dependency_graph_df$import <- ifelse(dependency_graph_df$package %in% dependencies, "Direct", "Indirect")
dependency_graph_df$import <- ifelse(dependency_graph_df$package == "papaja", "Mother", dependency_graph_df$import)
dependency_graph_df$alpha <- ifelse(dependency_graph_df$package == "papaja", 0.4, 0)
dependency_graph_df$face <- ifelse(dependency_graph_df$package == "papaja", "bold", "plain")

dependency_graph_df$n_dependencies <- table(gsub("\\|.+", "", attr(E(dependency_graph), "vnames")))[as.character(dependency_graph_df$package)] %>% as.vector
dependency_graph_df$n_dependencies[is.na(dependency_graph_df$n_dependencies)] <- 0


dependency_graph_df$importance <- table(gsub(".+\\|", "", attr(E(dependency_graph), "vnames")))[as.character(dependency_graph_df$package)] %>% as.vector
...
dependency_graph_df %>%
  ...
  geom_nodes(aes(color = n_dependencies, size = I(7 * importance))) +
  scale_color_distiller(palette = "Spectral") +
  scale_size(labels = function(x) abs(max_downstream_deps - ceiling(x / 7 * max_downstream_deps)), breaks = round(seq(0, max_downstream_deps, length.out = 7))) +
  theme_blank(legend.position = "top") +
  guides(
    size = guide_legend(title = "Downstream dependencies", title.position = "top", title.hjust = 0.5, label.position = "bottom", label.hjust = 0.5, nrow = 1)
