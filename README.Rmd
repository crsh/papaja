...
Function `apa_table()` may be used to include publication-ready tables in your manuscript.
...
knitr::kable(out, caption ="Table 1. *Mean pea yield (with standard errors)*", align = "c", digits = 2)
```

This is a fairly simple example, but `apa_table()` may be used to generate massive tables.

`apa_table()` is also able to include the output from `apa_print()` into the manuscript. Thus, it is possible to
conveniently include complete regression tables, ANOVA tables, or the output from mixed-effects models.
...
Be sure to also check out `apa_barplot()`, `apa_lineplot()`, and `apa_beeplot()` (or the general function `apa_factorial_plot()`)
...
RefManageR::WriteBib(papaja_bib[sample(seq_along(papaja_bib), size = 6L)], "inst/papaja_manuscripts.bib")
```
...
Below are some (randomly selected) peer-reviewed publications that used `papaja`.
