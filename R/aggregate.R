...
.aggregate<-function(data,factors,dv,fun,na.rm=FALSE){

  fl <- lapply(as.list(factors),FUN=as.name)

  grouped<-grouped_df(data,vars=fl,drop=TRUE)
  agg.data<-as.data.frame(summarise_each(grouped,funs(fun(.,na.rm=na.rm)),matches(dv)))
