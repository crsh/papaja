...
#'
#' Takes \code{glm} objects from various statistical methods to create formatted character
#' strings to report the results in accordance with APA manuscript guidelines.
#'
#' @param x \code{glm} object.
...
#'
#' @return
#'    \code{apa_print.glm} returns a list containing the following components according to the input:
#'
#'    \describe{
...
apa_print.glm <- function(
  ...
  apa_res$statistic <- apply(tidy_x[, -1], 1, function(y) {
    ...
    stat <- paste0("$", stat_name, "(", glance_x$df.residual, ") = ",  printnum(y["statistic"]), "$, $p ", eq, p, "$")
  ...
  variable_label(regression_table) <- c(
    ...
    , statistic = paste0("$", stat_name, "(", glance_x$df.residual, ")$")
    , p.value = "$p$"
  )
