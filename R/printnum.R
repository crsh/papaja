...
printnum <- function(x, digits = 2, gt1 = TRUE, zero = TRUE, margin = 1, na_string = "") {
  ...
  validate(digits, "digits", check.class = "numeric", check.integer = TRUE, check.range = c(0, Inf))
  validate(gt1, "gt1", check.class = "logical")
  validate(zero, "zero", check.class = "logical")
