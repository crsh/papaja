...
$if(affiliation)$
  ...
  \def\affinst{{$for(affiliation)$"$affiliation.institution$"$sep$, $endfor$}}%
  \def\affstate{{$for(affiliation)$"$affiliation.state$"$sep$, $endfor$}}%
  \def\affcntry{{$for(affiliation)$"$affiliation.country$"$sep$, $endfor$}}%
$endif$
    ...
    $for(author)$
      $if(author.corresponding)$
        $if(author.affiliation)$
          ...
          \addtocounter{author}{$author.affiliation$}
          \expandafter\edef\csname authorid\endcsname{\theauthor}
          Correspondence concerning this article should be addressed to $author.name$, \pgfmathparse{\affdep[\authorid]} \pgfmathresult, \pgfmathparse{\affinst[\authorid]} \pgfmathresult$if(author.address)$, $author.address$$endif$.$if(author.email)$ E-mail: $author.email$$endif$
        $endif$
      $endif$
    $endfor$
    $endif$
  }
$else$ % If no note is define give only author information if available
  $if(author)$
  \note{
    \vspace{1cm}
    Author note
