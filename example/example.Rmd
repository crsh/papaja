...
# How to use papaja
Once you have installed `papaja` and all other [required software](https://github.com/crsh/papaja), you can select the APA template when creating a new Markdown file through the RStudio menus, see Figure \@ref(fig:menu).
When you click RStudio's *Knit* button (see Figure \@ref(fig:knit)), `papaja`, `R Markdown,` and `knitr` work together to create an APA conform manuscript that includes both your text and the output of any embedded R code chunks within the manuscript.

```{r menu, fig.cap = "papaja's APA6 template is available through the RStudio menues."}
knitr::include_graphics("../inst/images/template_selection.png")
```

```{r knit, fig.cap = "The *Knit* button in the RStudio."}
knitr::include_graphics("../inst/images/knitting.png")
```
