---
title: "Getting started on developing new `apa_print()` methods"
author: "Frederik Aust"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Developing new apa_print() methods}
...
If you are just looking for a way to contribute, take a look at the [open issues](https://github.com/crsh/papaja/issues) for inspiration.
...
printnum(x, digits = 3, big.mark = ".", decimal.mark = ",")
...
The internal function `print_df()` is geared towards typesetting degrees of freedom.

```{r printdf}
papaja:::print_df(c(12, 12.485))

papaja:::print_df(12L)
```

Finally, `print_interval()` can be used to typeset interval estimates.

```{r print-interval}
print_interval(rnorm(2), conf_level = 0.95, interval_type = "CI")
```
...
papaja:::sanitize_terms(mod_terms, standardized = TRUE)
```
...
```{r prettify-terms}
papaja:::prettify_terms(mod_terms, standardized = TRUE)
```
...
We than typeset all "special" numerical results, that is, all results that would not be typeset appropriately by applying `printnum()` with its default settings.
Moreover, we combine the separate columns for lower and upper confidence interval bounds into on column `conf.int` which contains the complete confidence interval.
...
tidy_lm_fit$p.value <- printp(tidy_lm_fit$p.value)
tidy_lm_fit$conf.int <- unlist(print_confint(tidy_lm_fit[, c("conf.low", "conf.high")]))

str(tidy_lm_fit)

glance_lm_fit <- glance(lm_fit)
glance_lm_fit$r.squared <- printnum(glance_lm_fit$r.squared, gt1 = FALSE)
glance_lm_fit$p.value <- printp(glance_lm_fit$p.value)
glance_lm_fit$df <- papaja:::print_df(glance_lm_fit$df)
glance_lm_fit$df.residual <- papaja:::print_df(glance_lm_fit$df.residual)
...
tidy_lm_fit <- printnum(tidy_lm_fit)
...
glance_lm_fit <- printnum(glance_lm_fit)
...
In case, we need to add additional information to this output, we can use `add_glue_to_apa_results()`.
...
A final issue to consider is that users may pass inappropriate input to **papaja** `apa_print()`.
