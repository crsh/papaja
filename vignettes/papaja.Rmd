...
vignette: >
  %\VignetteIndexEntry{Prepare APA Journal Articles with R Markdown}
...
For one in four irreproducible results, the reproduction attempt yielded results that were no longer statistically significant calling the original statistical inference into question.
...
Both the Visual Editor and **citr** can also access your Zotero database and copy references to your bibliography file.
...
The function takes the R object returned by the analysis function and in turn returns a list that contains reportable text and tables.
